	...
	m_Layers(filepaths.size())
{
	...
	{
		m_Buffers[i] = stbi_load(m_Filepaths[i].c_str(), &m_Width, &m_Height, &m_Channels, 4);
	}
	...
	GlApiCall(glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
	GlApiCall(glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_S, GL_CLAMP));
	GlApiCall(glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_T, GL_CLAMP));
